...
class Enemy(Tank):
    ...
    def move(self):
        ...
        global players, enemies, bonuses, astar_map, costmap

        if self.state != self.STATE_ALIVE or self.paused or self.paralised:
            return

        #if(self.path == []):
        if self.rect.top % 16 == 3 and self.rect.left % 16 == 3:
            self.path = self.generatePath(direction=None, fix_direction=True)
        ...
        '''
        Collision detections, remove for now. maybe needed zmy
        ...
        while(pq.isEmpty()==False):
            ...
            for direction in self.directions:
                ...
                if self.visited_map[newPosition[1]][newPosition[0]] == 1 \
                        or astar_map[newPosition[1]][newPosition[0]] == 0:
                        continue
    ...
    def cost(self, node):
        ...
        '''
        astar_cost = max(astar_map[position[1]][position[0]], astar_map[position[1]+1][position[0]],
...
if __name__ == "__main__":
    ...
    costmap=[[1 for x in range(26)] for x in range(26)]
